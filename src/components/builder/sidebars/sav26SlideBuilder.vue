<template>
  <div class="px-2 pt-3 text-left">
    <div v-for="(sav26SlideData, i) in sav26SlideDraft.element" :key="i">
      ...
      <hr class="mb-2" />
      <div v-for="(subData, j) in sav26SlideData.subElement" :key="j">
...
export default {
  ...
  setup(props, { emit }) {
    const sav26SlideDraft = ref(cloneDeep(props.slideDynamicData))
    watch(
      sav26SlideDraft,
      () => emit('updateSlideDynamicData', sav26SlideDraft.value),
      {
        deep: true,
      },
    )
