<template>
  <div class="px-2 pt-3 text-left">
    <div v-for="(sav27SlideData, i) in sav27SlideDraft.element" :key="i">
      ...
      <hr class="mb-2" />
      <div v-for="(subData, j) in sav27SlideData.subElement" :key="j">
...
export default {
  ...
  setup(props, { emit }) {
    const sav27SlideDraft = ref(cloneDeep(props.slideDynamicData))
    watch(
      sav27SlideDraft,
      () => emit('updateSlideDynamicData', sav27SlideDraft.value),
      {
        deep: true,
      },
    )
